...
The _kernel_ is the lowest level part of an operating systems. We can see the kernel as the heart of the operating system.  
...
- Install the kernel:  
  %> *sudo make install*  
  This will install your new kernel into */boot*.  
  It will also upgrade the GRUB bootloader configuration file (*/boot/grub/grub.cfg*), so, next time we will start the system we can select our new kernel.  
- If you want to update the boot options, you can edit the */etc/default/grub* file than run:  
  %> *sudo update-grub*  

To remove a compiled kernel, you can perform the following steps:  
- Remove all the files related to that kernel from the */build* directory.   
- Upgrade the GRUB configuration file by running the command:  
  %> *sudo update-grub*  
  This will remove the kernel entry from GRUB's configuration file.  
